...
class Show1(BoxLayout):
    pass
class ShowVoiture1(GridLayout):
    def __init__(self,**kwargs):
        super(ShowVoiture1,self).__init__(**kwargs)
        self.cols = 3
        self.btns=[]
        ...
        for i in range(15):
            ...
            #if i==0 or i==1:
             #   self.btns[-1].background_color=(0,0,0,0)
    def afficher(self):
        self.btns[0].background_color = self.btns[1].background_color=(0,0,0,0)
        self.btns[0].text = self.btns[1].text =''

        self.liste = App.get_running_app().manager.ids.show.ids.boxlayoutshow.ids.show1.ids.liste
        id_nom = []
        self.listes = []
        self.height_label = 38
        ...
        for row in rows:
            if row[0]//15==0:
                ...
                if row[1] not in id_nom:
                    ...
                    self.listes.append(Label(text=f'{info_pers[3]%100} : {info_pers[0]}  {info_pers[1]}',size_hint=(1,None),height=self.height_label,halign='left',valign="middle"))
        ...
        app.manager.liste_voiture1 = id_nom
